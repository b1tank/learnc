...
int main(void) {
    /* The starter code that gets loaded into the editor.
       Make it minimal and runnable. */
    return 0;
}
```
...
- A short bulleted breakdown of each interesting piece of the starter code.
- Define new vocabulary the first time it appears, in **bold** or *italic*.
- Aim for 3–6 bullets per lesson.

## Modern note

A callout used whenever K&R's 1988 style differs from current C99+ practice
(e.g. `gets`, K&R-style function declarations, missing `void` parameter lists,
absence of `<stdbool.h>`, etc.). One short paragraph is enough.
