...
> **Source video.** [Let's Learn C — lesson 24](https://www.youtube.com/watch?v=-QxrmHo-V7Y) (originally *Corso di programmazione in C — lezione 24: l'interprete Toy Forth (parte 2)*) by Salvatore Sanfilippo.
...
1. Add an `int count = 0;` and print `total: N` after the loop. Verify it matches the four tokens above.
2. Replace the input with `"5\tdup\nprint"` and confirm `isspace` already handles tab and newline.
3. Reject tokens longer than 4 bytes: if `len > 4`, print `[too long]` instead of the slice.

## Go deeper

- [*Starting Forth*](https://www.forth.com/starting-forth/) by Leo Brodie — the friendliest introduction to the language Toy Forth is imitating; free online.
- Concatenative languages on Wikipedia (<https://en.wikipedia.org/wiki/Concatenative_programming_language>) — Joy, Factor, Cat, and the family Salvatore reminisces about contributing to in 2004.
- Sean Barrett's "stretchy buffer" trick (<https://github.com/nothings/stb/blob/master/stb_ds.h>) — a one-header implementation of the doubling-realloc array used by `listPush`, with the length stored in a header before the pointer.

*Click **next →** to finish the parser and start executing tokens.*
